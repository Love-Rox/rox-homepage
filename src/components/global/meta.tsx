...
export const Meta = ({ title, description, image, url, lang = 'en' }: MetaProps) => {
  ...
  const absoluteUrl = url ? (url.startsWith('http') ? url : `https://love-rox.cc${url}`) : undefined;
  ...
  return (
    <>
      ...
      {url && (
         <>
           <link rel="alternate" hrefLang="en" href={`https://love-rox.cc${url.replace(/^\/(ja|en)/, '/en')}`} />
           <link rel="alternate" hrefLang="ja" href={`https://love-rox.cc${url.replace(/^\/(ja|en)/, '/ja')}`} />
           <link rel="alternate" hrefLang="x-default" href={`https://love-rox.cc${url.replace(/^\/(ja|en)/, '/en')}`} />
         </>
      )}
